        ...
        if (recuperoStorage !== null) {
            ...
            for(let i = 0; i < favoritosToArray.length; i++){
                if(favoritosToArray[i] !== null){
                    fetch(`https://thingproxy.freeboard.io/fetch/https://api.deezer.com/album/${favoritosToArray[i]}`)
                    .then(res => res.json())
                    .then(data => {
                        peliculas.push(data)
                            ...
                            {this.state.peliculas.length > 0 ?
                                this.state.peliculas.map((unaPelicula, idx) =>
                                    <article className='movie-card' key={idx}>
                                        {console.log(unaPelicula, 'nashe')}
                                        <Link to={`/detallePelicula/id/${unaPelicula.id}`}>
                                            {console.log(this.state.peliculas, 'PELISSS')}
                                            <img src={unaPelicula.cover} alt="" />
                                        </Link>
